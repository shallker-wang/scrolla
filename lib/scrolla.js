  ...
  function listenResize(scrollbar) {
    if (scrollbar.direction === 'horizontal') {
      scroll.on('resize', function () {
        if (scroll.availableScrollWidth > 0) scrollbar.track.thumb.percentWidth = scroll.percentWidth;
      });
    }

    if (scrollbar.direction === 'vertical') {
      scroll.on('resize', function () {
        if (scroll.availableScrollHeight > 0) scrollbar.track.thumb.percentHeight = scroll.percentHeight;
      });
    }
  }
